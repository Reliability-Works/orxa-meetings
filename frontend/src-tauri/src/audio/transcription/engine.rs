...
pub async fn get_or_init_whisper<R: Runtime>(
    ...
    if let Some(engine) = existing_whisper_engine() {
        if can_reuse_loaded_whisper(app, &engine).await {
            return Ok(engine);
        }
    }

    let engine = initialize_whisper_engine().await?;
    let model_to_load = whisper_model_to_load(app).await?;
    load_whisper_model_or_fallback(&engine, &model_to_load).await?;

    Ok(engine)
}

fn existing_whisper_engine() -> Option<Arc<crate::whisper_engine::WhisperEngine>> {
...
async fn can_reuse_loaded_whisper<R: Runtime>(
    app: &AppHandle<R>,
    ...
    match configured_whisper_model(app).await {
        Some(expected_model) if current_model == expected_model => {
            info!(
...
async fn configured_whisper_model<R: Runtime>(app: &AppHandle<R>) -> Option<String> {
    match crate::api::api_get_transcript_config(app.clone(), app.clone().state(), None).await {
        Ok(Some(config)) => {
            info!(
...
async fn initialize_whisper_engine() -> Result<Arc<crate::whisper_engine::WhisperEngine>, String> {
    ...
    existing_whisper_engine().ok_or_else(|| "Failed to get initialized engine".to_string())
}

async fn whisper_model_to_load<R: Runtime>(app: &AppHandle<R>) -> Result<String, String> {
    match crate::api::api_get_transcript_config(app.clone(), app.clone().state(), None).await {
        Ok(Some(config)) => {
            info!(
...
async fn load_whisper_model_or_fallback(
    engine: &Arc<crate::whisper_engine::WhisperEngine>,
    model_to_load: &str,
) -> Result<(), String> {
    ...
    match model_info {
        Some(model) => load_discovered_whisper_model(engine, model_to_load, model).await,
        None => load_first_available_whisper_model(engine, model_to_load, &models).await,
    }
}

async fn load_discovered_whisper_model(
    engine: &Arc<crate::whisper_engine::WhisperEngine>,
    model_to_load: &str,
    model: &crate::whisper_engine::ModelInfo,
) -> Result<(), String> {
    match model.status {
        ...
        crate::whisper_engine::ModelStatus::Missing => Err(format!(
            "Model '{}' is not downloaded. Please download it first from the settings.",
            model_to_load
        )),
        crate::whisper_engine::ModelStatus::Downloading { progress } => Err(format!(
            "Model '{}' is currently downloading ({}%). Please wait for it to complete.",
            model_to_load, progress
        )),
        crate::whisper_engine::ModelStatus::Error(ref err) => Err(format!(
            "Model '{}' has an error: {}. Please check the model or try downloading it again.",
            model_to_load, err
        )),
        crate::whisper_engine::ModelStatus::Corrupted { .. } => Err(format!(
...
async fn load_first_available_whisper_model(
    engine: &Arc<crate::whisper_engine::WhisperEngine>,
    model_to_load: &str,
    ...
    let fallback_model = models
        .iter()
        .find(|model| matches!(model.status, crate::whisper_engine::ModelStatus::Available))
        .ok_or_else(|| {
            format!("Model '{}' is not supported and no other models are available. Please download a model from the settings.", model_to_load)
        })?;
