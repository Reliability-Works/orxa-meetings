...
function providerNeedsApiKey(data: any) {
  return data.provider !== "ollama" && data.provider !== "custom-openai" && !data.apiKey;
}

async function attachProviderApiKey(data: any) {
  if (!providerNeedsApiKey(data)) return;
...
async function attachCustomOpenAIConfig(data: any) {
  if (data.provider !== "custom-openai") return;
...
async function loadModelConfigFromDatabase() {
  ...
  await attachProviderApiKey(data);
  await attachCustomOpenAIConfig(data);
  return data;
}
...
export function useModelConfiguration({ serverAddress }: UseModelConfigurationProps) {
  ...
  useEffect(() => {
    const fetchModelConfig = async () => {
      setIsLoading(true);
      try {
        const data = await loadModelConfigFromDatabase();
        if (data) {
          setModelConfig(data);
        } else {
  ...
  const handleSaveModelConfig = useCallback(
    async (updatedConfig?: ModelConfig) => {
      try {
        ...
        await Analytics.trackSettingsChanged(
          "model_config",
          `${payload.provider}_${payload.model}`,
        );
      } catch (error) {
