...
const [
  version,
  bundleDir = "frontend/src-tauri/target/release/bundle/updater",
  outputFile = "latest.json",
  notes = "",
] = process.argv.slice(2);
const repository = process.env.GITHUB_REPOSITORY || "Reliability-Works/orxa-meetings";

if (!version) {
  console.error(
    "Usage: node generate-update-manifest-github.js <version> [bundle-dir] [output-file] [notes]",
  );
  console.error(
    'Example: node generate-update-manifest-github.js 0.0.1 frontend/src-tauri/target/release/bundle/updater latest.json "Release notes"',
  );
  process.exit(1);
}
...
bundleFiles.forEach((filename) => {
  ...
  if (
    ...
    name.includes("macos") ||
    name.includes(".dmg") ||
    (name.includes(".app") && name.includes(".tar.gz"))
  ) {
    if (
    ...
    name.includes(".exe") ||
    name.includes(".msi") ||
    (name.includes(".zip") && !name.includes("darwin") && !name.includes("macos"))
  ) {
    ...
    name.includes(".deb") ||
    (name.includes(".tar.gz") && !name.includes("darwin") && !name.includes("macos"))
  ) {
  ...
  if (platform && !platforms[platform]) {
    ...
    const githubUrl = `https://github.com/${repository}/releases/download/${versionDir}/${filename}`;
...
if (Object.keys(platforms).length === 0) {
  ...
  console.error(
    "Expected files with names containing: darwin, macos, windows, linux, .exe, .dmg, .app, .AppImage",
  );
  process.exit(1);
}
...
console.log(`   URL: https://github.com/${repository}/releases/new?tag=v${versionClean}`);
...
console.log(`   curl https://github.com/${repository}/releases/latest/download/latest.json`);
