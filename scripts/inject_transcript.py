...
Injects CSV-based transcript data into the Orxa SQLite database,
...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "Orxa"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "Orxa"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "Orxa"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "Orxa"
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into Orxa database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    parser.add_argument("--folder-path", "-f", default=None, help="Optional path to audio folder")
    ...
    if not db_path.exists():
        ...
        print("Make sure Orxa has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the Orxa sidebar.")
