...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.orxa\meeting_minutes.db",
        "$env:USERPROFILE\Documents\orxa\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/orxa-backend/*/backend/meeting_minutes.db"
    }
