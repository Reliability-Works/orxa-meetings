...
function BrowseDatabaseButton({
  ...
  const isDetecting = importState === "selecting" || importState === "detecting";
...
function DetectionResult({ detectedPath }: { detectedPath: string | null }) {
...
function ImportErrorMessage({
  ...
  if (importState !== "error" || !errorMessage) return null;
...
function ImportActions({
  ...
  return (
    <div className="flex flex-col gap-3 pt-2">
      ...
      >
        <ImportButtonContent importState={importState} />
      </button>
...
function ImportButtonContent({ importState }: { importState: ImportState }) {
...
export function LegacyDatabaseImport({ isOpen, onComplete }: LegacyDatabaseImportProps) {
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage(
          "No database found at selected location. Please select the Orxa folder, backend folder, or the database file directly.",
        );
  ...
  const canImport = Boolean(detectedPath && importState === "idle");
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">Welcome to Orxa!</DialogTitle>
          <DialogDescription className="text-base pt-2">
            Do you have data from a previous Orxa installation?
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              Select your previous Orxa folder, backend directory, or database file:
            </p>
          ...
          <DetectionResult detectedPath={detectedPath} />

          <ImportErrorMessage errorMessage={errorMessage} importState={importState} />
