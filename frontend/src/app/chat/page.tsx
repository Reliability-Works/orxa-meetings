...
function MessageBubble({ message }: { message: ChatMessage }) {
  ...
  return (
    <div className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className={`max-w-[78%] rounded-2xl px-4 py-3 text-sm leading-6 ${isUser ? 'bg-gray-900 text-white' : 'bg-white text-gray-900 shadow-sm ring-1 ring-gray-200'}`}>
          ...
          <details className="mt-3 rounded-lg border border-gray-200 bg-gray-50 p-2 text-xs text-gray-600">
            ...
            <div className="mt-2 space-y-2">
              {message.evidence.slice(0, 6).map((item) => (
                <div key={item.transcript_id} className="rounded-md bg-white p-2 ring-1 ring-gray-100">
          ...
          <div className="rounded-[22px] border border-gray-200 bg-white shadow-[0_12px_35px_rgba(15,23,42,0.08)]">
            ...
            <div className="flex items-center justify-between gap-3 px-4 pb-4">
              <div className="flex min-w-0 items-center gap-2">
                <Button type="button" variant="ghost" size="icon" className="h-9 w-9 rounded-full text-gray-500">
