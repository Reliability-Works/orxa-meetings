  ...
  return (
    <div className="space-y-5">
      <div className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <div className="flex items-center justify-between gap-4 border-b border-gray-100 px-4 py-3">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <Button variant="outline" size="sm" onClick={saveSources} disabled={isSaving || isScanning}>
              {isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
              Save
            </Button>
            <Button size="sm" onClick={reindex} disabled={isScanning || enabledCount === 0}>
              {isScanning ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCcw className="h-4 w-4" />}
              Reindex
            </Button>
          </div>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_260px]">
          ...
          <div className="max-h-[520px] overflow-y-auto">
            {results.length === 0 ? (
              ...
              results.map((result, index) => (
                <div key={result.id} className={`px-4 py-3 ${index > 0 ? "border-t border-gray-100" : ""}`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-[12px] text-gray-400">{formatDate(result.sessionDate || result.modifiedAt)}</span>
                      </div>
                      <div className="mt-1 truncate text-sm font-medium text-gray-950">{result.title}</div>
                      <div className="mt-1 line-clamp-3 text-[13px] leading-5 text-gray-600">{result.snippet}</div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-gray-200 bg-white p-4">
            ...
            />
            <Button className="mt-3 w-full" variant="outline" size="sm" onClick={loadActivity} disabled={isSearching}>
              Show day
            </Button>
          </div>
      ...
      <div className="flex items-start gap-2 rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-600">
        ...
        <p>
          Agent history stays local in Orxa’s SQLite database. This indexes paths you enable here; it does not connect
          to Jira, Linear, GitHub, Slack, or any external task manager.
        </p>
      </div>
    </div>
  );
