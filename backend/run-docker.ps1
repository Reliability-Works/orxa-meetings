...
$AppProjectName = "orxa-backend"
$AppContainerName = "orxa-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "orxa-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server orxa-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server orxa-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "orxa-backend"
        "backend" = "orxa-backend"
        "meeting" = "orxa-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "orxa-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, orxa-backend"
        ...
        $logArgs += @("whisper-server", "orxa-backend")
    }
