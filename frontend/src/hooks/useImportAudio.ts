...
import { useImportAudioListeners } from "./importAudioListeners";
...
export function useImportAudio({
  ...
  const selectFile = useCallback(async (): Promise<AudioFileInfo | null> => {
    ...
    try {
      ...
      setStatus("error");
      const errorMsg =
        typeof err === "string" ? err : err?.message || String(err) || "Failed to validate file";
      setError(errorMsg);
  ...
  const startImport = useCallback(
    async (
      ...
      try {
        ...
        setStatus("error");
        const errorMsg =
          typeof err === "string" ? err : err?.message || String(err) || "Failed to start import";
        setError(errorMsg);
