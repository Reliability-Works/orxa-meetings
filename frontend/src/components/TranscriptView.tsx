...
function TranscriptRow({ transcript, index, streamingTranscript, showConfidence }: any) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        ...
        <TranscriptText isStreaming={isStreaming} displayText={displayText} sizerText={sizerText} />
      </div>
    </motion.div>
  );
}

function TranscriptText({ isStreaming, displayText, sizerText }: any) {
  const text = (
    <div className="relative">
      <p className="text-base text-gray-800 leading-relaxed" style={{ visibility: "hidden" }}>
        {sizerText}
      </p>
      <p className="text-base text-gray-800 leading-relaxed absolute top-0 left-0">{displayText}</p>
    </div>
  );
  ...
  return (
    <div className="flex-1">
      {isStreaming ? (
        <div className="bg-gray-100 border border-gray-200 rounded-lg px-3 py-2">{text}</div>
      ) : (
        text
      )}
    </div>
