...
type RawMeetingSummaryLanguagePreference = MeetingSummaryLanguagePreference | string | null;
...
function readLanguageFallback(prefix: string, meetingId: string): string | null {
  if (typeof window === "undefined") return null;
  try {
    return normaliseLanguageCode(window.localStorage.getItem(fallbackKey(prefix, meetingId)));
  } catch {
    return null;
  }
}
...
export async function applyPinnedSummaryLanguageToMeeting(
  meetingId: string,
): Promise<string | null> {
...
export async function readCachedDetectedSummaryLanguage(meetingId: string): Promise<string | null> {
  const response = normalisePreferenceResponse(
    await invoke<RawMeetingSummaryLanguagePreference>("api_get_meeting_detected_summary_language", {
      meetingId,
    }),
  );
...
export async function saveCachedDetectedSummaryLanguage(
  ...
  if (response.storage === "local_fallback") {
    writeLanguageFallback(DETECTED_SUMMARY_LANGUAGE_FALLBACK_PREFIX, meetingId, normalised);
    return;
  }
